...
## Deploying With Render + Vercel

Use Render for the FastAPI backend and Vercel for the React frontend.

### 1. Deploy the backend on Render

This repo includes a Docker-based Render Blueprint in `render.yaml`. Make sure these files are committed:
...
In Render, choose **New +** -> **Blueprint**, connect your repo, and select the `render.yaml` file at the repo root. The Blueprint creates a Docker web service named `alphabot-api`.

TensorFlow can be memory-heavy, so the free instance type may crash during startup or prediction.

Render will prompt for the secret values marked `sync: false`:
...
`ALPACA_BASE_URL` defaults to paper trading, and `SECRET_KEY` is generated by Render automatically.
...
In Vercel, import the same GitHub repo and use these settings:
...
The included `frontend/vercel.json` rewrites client-side routes like `/portfolio` back to `index.html`.

### 3. Connect both sides

After Vercel gives you the production URL, confirm Render has:

```env
ALLOWED_ORIGINS=https://<your-vercel-app>.vercel.app
```

If you use Vercel preview deployments, add those preview URLs to `ALLOWED_ORIGINS` as a comma-separated list.

### Optional: Render-only deployment

The root `Dockerfile` still supports deploying the full app on Render only. For the Render + Vercel split, `render.yaml` uses `Dockerfile.render` instead.
