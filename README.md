...
This repo includes a Docker-based Render Blueprint in `render.yaml`. It deploys the app as one web service:

- The Docker build compiles the Vite frontend.
- FastAPI serves both the API and the built frontend from the same Render URL.
- The frontend uses same-origin API calls in production, so no separate frontend API URL is needed.

### 1. Push this repo to GitHub

Render deploys from a Git repository. Make sure these files are committed:
...
In Render, choose **New +** -> **Blueprint**, connect your repo, and select the `render.yaml` file at the repo root.

The Blueprint creates one Docker web service named `alphabot` on the `starter` plan. TensorFlow can be memory-heavy, so the free instance type may crash during startup or prediction.
...
If you later split the frontend into a separate Render Static Site, set the frontend build environment variable:

```env
VITE_API_URL=https://<your-api-service>.onrender.com
```

And set the backend environment variable:
